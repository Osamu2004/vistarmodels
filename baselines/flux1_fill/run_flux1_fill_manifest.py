...
def prompt_for(direction: str, class_name: str) -> str:
...
def main() -> None:
    ...
    for row_index, row in enumerate(tqdm(rows, desc="FLUX.1 Fill-dev SECOND records")):
        ...
        if class_id > 0:
            if not bool((semantic_ids == class_id).any()) or not bool((np.asarray(binary_mask) > 0).any()):
        ...
        elif bool((np.asarray(binary_mask) > 0).any()) or bool((semantic_ids > 0).any()):
        ...
        prompt = prompt_for(str(row["direction"]), class_name)
